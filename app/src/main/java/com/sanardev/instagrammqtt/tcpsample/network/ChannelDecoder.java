 ...
 */
package com.sanardev.instagrammqtt.tcpsample.network;
...
public class ChannelDecoder extends FrameDecoder {
	...
	private void printPacket(byte[] packet) {
		...
		Log.i("TEST_APPLICATION",sb.toString());
	}
	...
	protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer) throws JSONException {
		...
		try {
			...
			try {
				...
				Log.i("TEST_APPLICATION","decrypt fail, cause" + e.getMessage());
			...
			if (decryptMessage.length < 4) {
				channel.close();
				Log.i("TEST_APPLICATION","decripted message should longer than 4 bytes, size = " + decryptMessage.length);
			...
			Log.i("TEST_APPLICATION",String.format("READ  [%s]", jsonString));
