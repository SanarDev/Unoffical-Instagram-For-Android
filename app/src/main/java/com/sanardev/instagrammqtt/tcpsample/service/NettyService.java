package com.sanardev.instagrammqtt.tcpsample.service;
...
import com.sanardev.instagrammqtt.tcpsample.network.ChannelDecoder;
import com.sanardev.instagrammqtt.tcpsample.network.ChannelEncoder;
import com.sanardev.instagrammqtt.tcpsample.network.NetworkEventHandler;
import com.sanardev.instagrammqtt.tcpsample.util.ThreadManager;
import com.sanardev.instagrammqtt.tcpsample.util.WakeLockWrapper;
...
public class NettyService extends WorkerService {
	...
	boolean checkConnection() {
		...
		Log.i("TEST_APPLICATION",NettyService.class.getSimpleName() + ".checkConnection(), mChannel: " + mChannel + ", result: " + result);
		return result;
	}
	...
	public void scheduleToReconnect() {
		...
		if(INTERVAL_RECONNECT_EXPONENTIAL_BACKOFF < INTERVAL_RECONNECT_MAXIMUM) {
			...
			Log.i("TEST_APPLICATION",String.format("%s.scheduleToReconnect() delay: %dsec", NettyService.class.getSimpleName(), INTERVAL_RECONNECT_EXPONENTIAL_BACKOFF/1000));
